...
def plot_losses(d_losses, g_losses, title):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(f"result/gan/losses.png")
...
def train_gan(df):
    ...
    while True:
        ...
        for i, real_data in enumerate(dataloader):
            ...
            fake_data_clone[:, 0] = torch.round(fake_data_clone[:, 0] * 65535)
            fake_data_clone[:, 1] = torch.round(fake_data_clone[:, 1] * 17)
            ...
            if (i + 1) % 1_000 == 0:
                ...
                print(f"\rStep {i + 1:10d} / {len(dataloader):10d} | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f}", end="")

        end_time = time.time()
        print("\r" + " " * 100, end="")
        print(f"\rFirst Epoch | Time: {end_time - start_time:.2f}s | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f}")
        if d_losses[-1] < 100:
            print(f"---d_losses[-1] < 100---")
            break

    total_epochs = int(MAX_TIME // (end_time - start_time) + 1)

    phase = total_epochs // 10
    if phase == 0:
        phase = 1
    ...
    for epoch in range(total_epochs):
        ...
        for i, real_data in enumerate(dataloader):
            ...
            if (i + 1) % 1_000 == 0:
                ...
                print(f"\rEpoch {epoch + 1:7d} / {total_epochs:7d} | Step {i + 1:10d} / {len(dataloader):10d} | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f}", end="")

        end_time = time.time()
        print("\r" + " " * 100, end="")
        print(f"\rEpoch {epoch + 1:7d} / {total_epochs:7d} | Time: {end_time - start_time:.2f}s | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f}")

        if phase > 0 and (epoch + 1) % phase == phase - 1:
            title = f"Epoch {epoch + 1:7d} / {total_epochs:7d}"
            plot_losses(d_losses, g_losses, title)
            print(f"---loss plot done---")
    else:
        title = f"Epoch {epoch + 1:7d} / {total_epochs:7d}"
        plot_losses(d_losses, g_losses, title)
...
if __name__ == "__main__":
    ...
    if is_train:
        for label in label_list:
            ...
            with open("result/gan/train_done.txt", "w") as f:
                f.write("train done\n")
                f.write(f"now time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
